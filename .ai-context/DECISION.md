...
- [19. ADR-019: Collapse the Anemic Runtime Cost/Effect/Ability Layer](#19-adr-019-collapse-the-anemic-runtime-costeffectability-layer)
- [20. ADR-020: Unify the Card Parse Pipeline on Typed Results](#20-adr-020-unify-the-card-parse-pipeline-on-typed-results)
- [21. ADR-021: Thin-Slice Resequencing of Infrastructure and AI Phases](#21-adr-021-thin-slice-resequencing-of-infrastructure-and-ai-phases)
...
- **Superseded in part (2026-07-18):** The infrastructure and AI phases were thin-sliced and resequenced (ADR-021): AI Strategies is split into Phase 7 (First Agent, ISMCTS) and Phase 9 (Advanced Agents, WorldModel then Hybrid), with Phase 6 (AI Enablement) before them and Phase 8 (Observability & Replay) between them. The "now Phase 8" AI-strategies mapping above becomes Phase 7 plus Phase 9. Stack-split rationale unaffected.
...
- **Superseded in part (2026-07-18, ADR-021):** The "Phase 6 AI milestone" phrasing predates the 9-phase resequencing; the AI milestone is now Phase 7 (First Agent). Replacement effects (Rule 614/615) and the layer system (Rule 613) remain deferred past all AI phases. The deferral decision and the TASK_0403 event-interception seam are unchanged.
...
- **Superseded in part (2026-07-18, ADR-021):** TASK_0601 stays in Phase 6, now named "AI Enablement." Its tee-decorator's decision-log-replay consumer is renumbered TASK_0702 to TASK_0804 (moved to Phase 8). The determinism model itself is unaffected.
...
- **Superseded in part (2026-07-18, ADR-021):** The event-trace and replay tasks are renumbered and rehomed into Phase 8 (Observability & Replay): TASK_0602 becomes TASK_0801 (event taxonomy), TASK_0603 becomes TASK_0802 (observer redesign plus JSONL sink), TASK_0701 becomes TASK_0803 (seed-replay plus validation hash), and TASK_0702 becomes TASK_0804 (decision-log replay). The trace and replay design is unchanged.
...
- **Superseded in part (2026-07-18, ADR-021):** The deep-clone seam TASK_0703 is renumbered TASK_0602 and moves into Phase 6 (AI Enablement) as part of the ISMCTS substrate, with its contract widened to clone-with-optional-determinization (a masked fork that hides opponent hand and library for hidden-information search). Clone performance tuning now belongs to the Phase 7 (First Agent) and Phase 9 (Advanced Agents) AI work rather than "the AI phase (now Phase 8)." The clone design is otherwise unchanged.

## 19. ADR-019: Collapse the Anemic Runtime Cost/Effect/Ability Layer

- **Date:** 2026-07-18
- **Status:** Proposed
- **Decision:** Eliminate the runtime `Engine.Cost`, `Engine.Effect`, and `Engine.Ability` data classes and the `IParameter`/`Parameter`/`ParameterKey` value bag. Handlers dispatch on `Kind` (as they already do) and read the typed `DefinedBlob` subclass directly, making `DefinedBlob` the single canonical card definition consumed by the engine. Runtime types are kept or introduced only where genuine per-instance state or behavior exists.
- **Context:** For every domain noun the codebase declares parallel families — `UnparsedBlob` (raw), `DefinedBlob` (parsed, serializable), and `Engine` (runtime). The runtime `Cost`/`Effect`/`Ability` carry only a `Kind` enum plus an `IParameter` bag keyed by `ParameterKey` (only `Amount` exists today); all behavior lives in handlers (`BaseCostHandler`/`BaseEffectHandler` with `PayingManaHandler`, `ProducingManaHandler`, `TappingHandler`). `EntityFactory` converts a typed `DefinedBlob.PayingManaCost` into `Engine.Cost { Parameter[Amount] = ManaBlob }`, and the handler immediately casts it back out via `FindValue<IManaCost>(Amount)` — a typed to untyped to typed round-trip that erases the parse result.
- **Rationale:** A runtime type earns its keep only when it holds per-instance state or behavior the definition must not. `Permanent` does (tapped, damage); `Engine.Cost/Effect/Ability` do not — they re-declare the definition and hold nothing extra, so they cost an extra family plus a mapper for zero benefit and introduced the stringly-typed `Parameter` bag as collateral. The axis that matters is definition-vs-instance, not serializable-vs-runtime. `DefinedBlob` is already a clean typed hierarchy (abstract base plus subclasses), not Scryfall-shaped — the anti-corruption boundary is `UnparsedBlob` — so consuming it in the engine adds no new coupling since `Kvasir.Engine` already references `Kvasir.Contract`.
- **Consequences:** Delete `Engine.Cost`, `Engine.Effect`, `Engine.Ability`, `IParameter`, `Parameter`, and `ParameterKey`. Handlers take the typed `DefinedBlob` subclass; the `ManaBlob` conversion (dictionary to `IManaCost` with behavior) moves into the handler or a thin typed adapter. `EntityFactory`'s cost/effect/ability conversion switch (which throws on unhandled kinds) largely disappears. `Permanent` references the card definition, and `CreaturePart` stops duplicating base power/toughness — base stats live on the definition, the part holds only `Damage` and counters, with effective power/toughness computed. When Phases 3-4 introduce chosen targets, X, or modes, add a `StackedEffect`/`StackedAbility` instance wrapper over the defined element rather than reviving the `Parameter` bag. `DefinedBlob` stays both the on-disk contract and the engine-consumed definition; a separate serialization contract is re-introduced only if a save/replay format must freeze independently or `DefinedBlob` accretes serialization cruft — both align with the Phase 8 replay work (ADR-017).
- **Open question:** Dependency direction — whether handlers consume the concrete `DefinedBlob` subclasses directly (simplest, tightest) or through a minimal runtime interface over them (preserves a `Contract`-to-`Engine` seam at the cost of one thin abstraction). To be settled before implementation.
- **Relation:** Reinforces the definition/instance boundary already correct for `Permanent`. Pairs with ADR-020 (parse-spine unification). Refactor is currently unscheduled; see `BACKLOG/DESIGN_Card_DataPipeline.md`.

## 20. ADR-020: Unify the Card Parse Pipeline on Typed Results

- **Date:** 2026-07-18
- **Status:** Proposed
- **Decision:** Consolidate the `UnparsedBlob.Card` to `DefinedBlob.Card` transformation onto a single mechanism — the generic, immutable `ParsingResult<T>` plus ANTLR grammar rules with per-rule visitors, building values through typed builders. Retire the reflection-based `ProcessingResult` machinery (`ProcessingResult`, `ValidProcessingResult`, `InvalidProcessingResult`, and the `BindTo(PropertyInfo)` binding). Move the remaining regex-parsed card fields (type line, mana cost, power/toughness) into grammar rules per ADR-009.
- **Context:** `MagicCardProcessor` maps card fields via a non-generic `ProcessingResult` holding `object`, a fluent `.ThenParseX()` chain, and reflection `propertyInfo.SetValue` onto init-only record properties — then calls into `MagicCardParser` (ANTLR), which already uses a clean generic `ParsingResult<T>` with typed visitors and builders, and re-wraps those typed results back into untyped ones. Two result types with overlapping roles operate on adjacent segments of one pipeline; the code's own TODO flags the confusion.
- **Rationale:** Reflection binding defeats records — it mutates `init` properties post-construction, boxes every `ushort`/`uint`/enum through `Expression<Func<Card, object>>`, has no compile-time safety, and silently binds default on an invalid child. The ANTLR/visitor/`ParsingResult<T>` half is already the right model — typed, immutable, char-accurate errors, one visitor per grammar rule — and scales to hundreds of effect forms as the grammar grows. Unifying on it removes a whole result type and the reflection layer and aligns with ADR-009 (one grammar-based path for all oracle text). Building `DefinedBlob.Card` via a `Card.Builder` matches the existing `PayingManaCost`/`ProducingManaEffect` builder pattern.
- **Consequences:** `MagicCardProcessor` composes typed `ParsingResult<T>` field parsers and assembles `DefinedBlob.Card` via a builder; card-level failures aggregate child failures via the existing `ParsingResult.CreateFailure(params ParsingResult[])`. Delete the `ProcessingResult` family and `BindToDefinedCard`. Type line, mana cost, and power/toughness become grammar rules, removing `Pattern.Card.Type`/`ManaCost` and the second-face regex TODO. Fix the `ThenParseText` fall-through (it discards the `WithMessage` return and binds `NotSupported` as if valid) as part of the migration. Combined with ADR-019, the end-to-end pipeline reduces to two mechanisms: a hand-mapped anti-corruption fetch (JSON to `UnparsedBlob`) and a single ANTLR parse spine (`UnparsedBlob` to `DefinedBlob`), both producing the one canonical `DefinedBlob`.
- **Relation:** Executes ADR-009 (unified ANTLR parsing) at the transformation-spine level. Pairs with ADR-019 (which removes the third, `DefinedBlob`-to-`Engine`, mechanism). Unscheduled; see `BACKLOG/DESIGN_Card_DataPipeline.md`.

## 21. ADR-021: Thin-Slice Resequencing of Infrastructure and AI Phases

- **Date:** 2026-07-18
- **Status:** Accepted
- **Decision:** Resequence the execution-infrastructure and AI work into a 9-phase roadmap, applying the same thin-slice principle (ADR-010) to infrastructure that was applied to keywords. Phase 6 "AI Enablement" builds only the ISMCTS substrate (master seed plus split RNG streams, TASK_0601; masked deep-clone seam, TASK_0602 — formerly TASK_0703). Phase 7 "First Agent (ISMCTS)" stands up the first agent on that substrate. Phase 8 "Observability & Replay" (typed event taxonomy TASK_0801, observer plus JSONL sink TASK_0802, seed-replay plus validation hash TASK_0803, decision-log replay TASK_0804 — formerly TASK_0602/0603/0701/0702) follows. Phase 9 "Advanced Agents" builds WorldModel then Hybrid.
- **Context:** The prior roadmap (ADR-016/017/018) front-loaded two full execution-infrastructure phases — Phase 6 Determinism & Observability and Phase 7 Replay & Cloning — before any AI phase. That built determinism, the full event trace, the JSONL sink, replay, and cloning before a single agent existed, deferring the payoff (a playing agent) behind a large infrastructure block and instrumenting behavior no agent had exercised yet.
- **Rationale:** Same thin-slice logic as ADR-010: when the payoff is gated on a later capability, build only the slice the next step strictly needs, then jump to the payoff. ISMCTS needs only reproducible RNG and a masked clone seam — not the event trace, the JSONL sink, or replay — so Phase 6 builds just that substrate and Phase 7 delivers a working agent. Observability and replay are then built with knowledge of what a real agent needs instrumenting, and are placed before Phase 9 because the WorldModel's self-play training pipeline (ADR-008) consumes the event trace as training data. Phase number still encodes execution order (ADR-015) with no phase gated on a later one (6 to 7 to 8 to 9). The task renumber is safe under ADR-011 — all affected tasks are not-started and referenced only inside the atomically-edited planning folder.
- **Consequences:** Nine phases; Phases 1-5 are unchanged. TASK_0703 (deep-clone seam) moves to Phase 6 as TASK_0602 with its contract widened to clone-with-optional-determinization (a masked fork for hidden information, which ISMCTS needs). The event-taxonomy, observer/JSONL, seed-replay, and decision-log tasks move to Phase 8 as TASK_0801-0804. WorldModel and Hybrid become Phase 9 (task breakdown pending, tracked in `BACKLOG/DESIGN_AI_AdvancedAgent.md`). Supersede-in-part notes are recorded on ADR-013 (the AI-strategies mapping is now Phase 7 plus Phase 9), ADR-016 (TASK_0601 stays in Phase 6; its decision-log-replay consumer is renumbered to TASK_0804), ADR-017 (the event-trace and replay tasks are renumbered into Phase 8), and ADR-018 (the clone seam is renumbered TASK_0602 in Phase 6). Escape hatch retained: the event-taxonomy plus JSONL sink (TASK_0801-0802) may be pulled forward into Phases 3-4 if stack/trigger debugging needs it, without moving the seed, clone, or replay tasks.
- **Relation:** Extends ADR-010 (thin-slice resequencing) to execution infrastructure; reaffirms ADR-015 (phase number encodes execution order); renumbers tasks under ADR-011. Supersedes in part the phase and task mappings of ADR-013, ADR-016, ADR-017, and ADR-018 (their technical decisions are unaffected).
