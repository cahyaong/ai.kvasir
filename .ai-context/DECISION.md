...
- **Relation:** Reinforces the definition/instance boundary already correct for `Permanent`. Pairs with ADR-020 (parse-spine unification). Refactor is currently unscheduled; see `BACKLOG/DESIGN_Card_Data_Pipeline_Architecture.md`.
...
- **Relation:** Executes ADR-009 (unified ANTLR parsing) at the transformation-spine level. Pairs with ADR-019 (which removes the third, `DefinedBlob`-to-`Engine`, mechanism). Unscheduled; see `BACKLOG/DESIGN_Card_Data_Pipeline_Architecture.md`.
